...
class NotificationRepoImpl {
    class FirestoreNotificationRepository(
         ...
         * fields: userId, userName, action, isRead, createdAt
         */
        ...
        override fun observeNotificationsForUser(userId: String): Flow<List<NotificationItem>> =
            callbackFlow {
                val listener = collection
        ...
        override suspend fun markAllAsReadForUser(userId: String) {
            val snap = collection.whereEqualTo("userId", userId).whereEqualTo("isRead", false).get()
                .await()
            val batch = db.batch()
